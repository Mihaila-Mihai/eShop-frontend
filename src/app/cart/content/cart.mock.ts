import {Cart, CartItem, Courier, DeliveryAddress, DeliveryMethod, Item, ProductSize} from "./model";


export const ITEM: Item = {
  id: "1",
  price: 30,
  color: "blue",
  image: "https://static.footshop.com/931018-full_product/264595.jpg",
  size: ProductSize.S,
  displayName: "TANK TOP DAILY PAPER REENA TOP",
  quantity: 1
}

export const CART_ITEMS: CartItem =
  {
    item: ITEM,
    quantity: 1
  }
;

export const DELIVERY_ADDRESS: DeliveryAddress = {
  firstName: "John",
  lastName: "Doe",
  email: "john.doe@gmail.com",
  phoneNumber: "407601231231",
  city: "Bucuresti",
  country: "Romania",
  street: "Mihail Kogalniceanu, 26",
  district: "Bucuresti",
}
export const CART_MOCK: Cart = {
  items: {
    "2": CART_ITEMS
  },
  courier: Courier.FAN_COURIER,
  deliveryMethod: DeliveryMethod.UNSPECIFIED,
  deliveryAddress: DELIVERY_ADDRESS,
  shippingInsurance: {
    applied: true,
    price: 15
  },
  voucher: {
    id: "DISCOUNT10",
    value: 10,
  },
  summary: {
    itemsCount: 1,
    productsTotalPrice: 30,
    totalPrice: 30
  }
}
